...
from payments.models import DataManifestStateEntry, RunnerEntry
from django.http import JsonResponse, Http404

def get_runner_manifest(current_manifest_version):
    ...
    articles = RunnerEntry.objects.filter().values(
        "id",
        "updated_at",
    )
...
def get_runner_article(article_uid):
    try:
        article = RunnerEntry.objects.get(id=article_uid)
